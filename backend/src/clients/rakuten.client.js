 ...
 * - 라쿠텐 Web Application 타입은 Referer/Origin 출처 검증을 요구한다.
 *   fetch는 Referer/Origin을 forbidden header로 막으므로 node:https로 직접 전송한다.
 */
...
async function search(params) {
  ...
  let res;
  try {
    res = await httpsGet(url, config.rakuten.referer, config.rakuten.timeoutMs); // BR-003 (5초)
  } catch (e) {
    if (e.code === 'TIMEOUT') {
  ...
  const json = (() => { try { return JSON.parse(res.body); } catch { return null; } })();

  const product = json?.Products?.[0]?.Product;
  return product ? mapToDTO(product) : null; // 결과 없으면 null
}

function httpsGet(url, referer, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(url, { method: 'GET', headers }, (response) => {
      let body = '';
    ...
    req.setTimeout(timeoutMs, () => {
      const err = new Error('rakuten timeout');
      err.code = 'TIMEOUT';
      req.destroy(err);
      reject(err);
    });
