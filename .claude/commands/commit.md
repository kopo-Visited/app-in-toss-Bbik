...
argument-hint: <F-ID 또는 주제> 예) F-004 저장 / docs erd
---

다음 순서로 `$ARGUMENTS` 변경을 커밋하라. `git-workflow` 스킬을 따른다.

1. **현재 브랜치 확인**: main 또는 develop이면 **중단**하고 경고한다(feature/문서 브랜치에서 작업해야 함).
   - 새 브랜치가 필요하면 네이밍 규칙대로 생성 제안:
     - 백엔드 기능 `feat/be/<F-ID>-<요약>` / 프론트 기능 `feat/fe/<F-ID>-<요약>`
     - 버그 `fix/be|fe/<F-ID>-<요약>` / 문서 `docs/<주제>` / 설정 `chore/<주제>`
   - develop에서 분기: `git switch develop && git pull && git switch -c <브랜치명>`.
...
4. 변경의 F-ID/BL-ID(문서면 주제) 식별 후 Conventional Commit 메시지 작성: `<type>(<scope>): <요약>`.
5. 커밋 생성. 1커밋=1관심사가 아니면 분리 제안. **한 브랜치에 여러 기능 섞지 말 것.**
